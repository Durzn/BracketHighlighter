...
suite('The Extension will search at the cursor and ', () => {
    ...
    test('highlight the correct range even if it is not the first text in the line.', () => {
        /* Environment */
        let editor = vscode.window.activeTextEditor!;
        assert.notStrictEqual(editor, undefined);

        /* Setup */
        let symbol = new HighlightSymbol(new HighlightEntry("do", false, false), new HighlightEntry("end", false, false), JumpBetweenStrategy.TO_SYMBOL_OPPOSITE_SIDE); /* '/*' symbol expected */
        configCache.configuredSymbols = [symbol];
        configCache.ignoreContent = true;
        let selectionStart: vscode.Selection = new vscode.Selection(new vscode.Position(71, 0), new vscode.Position(71, 0));

        /* Expectation setup */
        let rangeOpen = new vscode.Range(new vscode.Position(70, 20), new vscode.Position(70, 20 + symbol.startSymbol.symbol.length));
        let rangeClose = new vscode.Range(new vscode.Position(73, 0), new vscode.Position(73, symbol.endSymbol.symbol.length));
        let expectedRange: SymbolAndContentRange = new SymbolAndContentRange(
            [rangeOpen, rangeClose],
            undefined);
    ...
    test('highlight the correct range if the symbol is a regular expression.', () => {
        ...
        /* Expectation setup */
        let symbol = ConfiguredSymbols[7]; /* '\\bdo\\b(?!:)' symbol expected */
    ...
    test('highlight the correct range if the symbol is a regular expression and supports multiple ending symbols.', () => {
        ...
        /* Expectation setup */
        let symbol = ConfiguredSymbols[6]; /* 'pthread_mutex_lock' symbol expected */
        assert(symbol.startSymbol.isRegex);
...
suite('The Extension will search from the scope ', () => {
    ...
    test('highlight the correct range if the symbol is a regular expression part 2.', () => {
        /* Environment */
        let editor = vscode.window.activeTextEditor!;
        assert.notStrictEqual(editor, undefined);
        ...
        let symbol = ConfiguredSymbols[7]; /* '\\bdo\\b(?!:)' symbol expected */
        assert(symbol.startSymbol.isRegex);
        let rangeOpen = new vscode.Range(new vscode.Position(70, 20), new vscode.Position(70, 22));
        let rangeClose = new vscode.Range(new vscode.Position(73, 0), new vscode.Position(73, 3));
        let expectedRange: SymbolAndContentRange = new SymbolAndContentRange(
            [rangeOpen, rangeClose],
            undefined);
    ...
    test('highlight the correct range if there are multiple configured symbols on the same line.', () => {
        ...
        /* Expectation setup */
        let symbol = ConfiguredSymbols[3]; /* '{' symbol expected */
...
suite('The Extension will work with multiple cursors ', () => {
    ...
    test('and highlight each range', () => {
        ...
        /* Expectation setup */
        let symbol1 = ConfiguredSymbols[7]; /* '\\bdo\\b(?!:)' symbol expected */
        let symbol2 = ConfiguredSymbols[3]; /* '{' symbol expected */
        let rangeOpen1 = new vscode.Range(new vscode.Position(39, 0), new vscode.Position(39, 2));
        let rangeClose1 = new vscode.Range(new vscode.Position(44, 0), new vscode.Position(44, 3));
